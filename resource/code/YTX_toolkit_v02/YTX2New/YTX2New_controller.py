...
class CFXloadYeti():
    ...
    def query_pubs(self, project, assetname):
        files = []
        if self.__PIPESTEP__ == "characterfx":
            tar_taskname = "cfx"
        else:
            tar_taskname = "hair"
        filters = [['project.Project.name', 'is', project],
                    {
                        'filter_operator' : 'all',
                        'filters': [
                                    ['code', 'contains',  assetname+"_"],
                                    ['code', 'not_contains',  "_basemesh"],
                                    ['code', 'contains',  tar_taskname],
    ...
    def modify_info(self, sg_info_dict: dict) -> set:
        def make_json_paths(maya_path: str) ->  set:
            ...
            gRest_json_full_path = re.sub(".mb$", "_gRest.json", temp_path)
            return json_full_path, attr_json_full_path, gRest_json_full_path

        temp_path           = sg_info_dict['path']['url']
        pub_path            = re.sub("^file:////", "/", temp_path)
        pub_json_path, pub_json_attr_path, pub_gRest_path = make_json_paths(pub_path)
        ...
        return pub_path, pub_json_path, pub_json_attr_path, pub_gRest_path, desc, task_name, vernum, created_at, created_by, id

    
    def sg_2_pubInfo(self, pub_sg_list: list) -> Generator:
        for pub_info_dict in pub_sg_list:
            pub_path, pub_json_path, pub_json_attr_path, pub_gRest_path, desc, task_name, vernum, created_at, created_by, id = self.modify_info(pub_info_dict)
            yield pubInfo(pub_path, pub_json_path, pub_json_attr_path, pub_gRest_path, desc, task_name, vernum, created_at, created_by, id)
    ...
    def do_assign(self, selected_pubinfo: pubInfo, **kwargs):
        # def get_only_setname(_infos :list) -> list:
        #     res = []
        #     for _info in _infos:
        #        res.append(_info["EACH_SET"]) 
        #     return res
        ...
        gRest_json_full_path= selected_pubinfo.pub_gRest_path
        ...
        for asset_name, cfx_data in list(self._model.items()):
            ...
            for cur_yeti in yeti_info_list:
                ...
                grm_path_list = cur_yeti_info_dict.get("GRM_DATA")
                if os.path.exists(grm_path_list[0]) == True:
                    grm_path = grm_path_list[0]
                else:
                    grm_path = grm_path_list[1]
            ...
            for _assetname_GRP in all_asset_tar_list:
                ...
                res, renamed_set_names = YT.do_assign(_assetname_GRP, y_info_hub)
                whole_cv_set_list.extend(renamed_set_names)
                if res == False:
        ...
        if os.path.exists(gRest_json_full_path) == True:
            print(whole_cv_set_list)
            YT.save_guide_rest_pose(whole_cv_set_list)
            
            yt_py.import_set_restpose(gRest_json_full_path)
    ...
    def do_furCache_assign(self, *args, **kwargs) -> None:
        ...
        def show_confirm_dialog(msg :str) -> None:
            confirm_dialog = QtGui.QMessageBox()
            confirm_dialog.setWindowTitle("YTX2")
            confirm_dialog.setText(msg)
            confirm_dialog.setDefaultButton(QtGui.QMessageBox.Ok)
            apply_stylesheet(confirm_dialog, "dark_red.xml")
            confirm_dialog.exec_()


        def select_confirm_dialog(msg :str) -> None:
            select_dialog = QtGui.QMessageBox()
            select_dialog.setWindowTitle("YTX2")
            select_dialog.setText(msg)
            select_dialog.setStandardButtons(QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
            apply_stylesheet(select_dialog, "dark_red.xml")
            if select_dialog.exec_() == QtGui.QMessageBox.Yes:
                return True
            else:
                return False
        ...
        sg_pub_info = args[0]
        fur_dir     = args[1][0]
        print(f"Fur Dir : {fur_dir}")
        
        file_list   = os.listdir(fur_dir)
        

        if file_list == []:
            show_confirm_dialog(f"해당 경로에 .fur 파일이 없습니다.\n{fur_dir}")
            return
        
        is_it_right = False
        if file_list != []:
            for _file in file_list:
                if self.cur_assetname in _file:
                    is_it_right = True
                    break
        if is_it_right == False:
            show_confirm_dialog(f"해당 경로에 {self.cur_assetname} .fur 파일이 없습니다.\n{fur_dir}")
            return
        ...
        for _fname in file_list:
            ...
            yeti_name  = only_fname.replace(asset_ns+"_", "")
            yeti_name  = re.sub(r"(\.)", "", yeti_name)
        ...
        # Step 02. get .ma asset pub filepath and .json asset pub filepath and rearrange file_info_hub data
        ...
        yeti_node_count = 0
        grm_cache_info = {}
        for y_shape_name, yeti_info_dict in yeti_json_info.items():
            if y_shape_name in ['MDL_PUB_VER', 'MDL_DEV_VER','JSON_DATA', 'MA_DATA', '']:
                continue
            yeti_node_count += 1
            if yeti_info_dict.get("IS_CURVE_VER") == False:
                grm_cache_info[y_shape_name] = yeti_info_dict

        
        pprint(all_targets)
        pprint(file_info_hub)
        
        # Step 05.
        #       - check yeti info and if the yeti include curve data, use .fur cache 
        #         (if yeti do not include curves, use .grm cache)
        #       - find right basemesh
        #       - create yeti node 
        #       - set .fur path / image search path
        #       - assign material
        for fname_prefix, fur_cache_info in list(file_info_hub.items()):
            # print(3333333333, fur_cache_info.ns_number)
            ...
            with_curve          = yeti_node_in_json.get("IS_CURVE_VER")
            grm_cache_path      = ""
            # if with_curve       == False:
            #     continue
            ...
            for shape_texRef_info in shape_texRef_pair_list:
                ...
                if cur_ns_name not in all_targets:
                    ani_nses = "/".join(list(all_targets.keys()))
                    _msg = f"fur 캐쉬와 Ani 캐쉬의 namespace가 다릅니다.\nFur cache : {cur_ns_name}\nAni cache:{ani_nses}\n\nfur 캐쉬파일 기준으로 진행하시겠습니까?"
                    res = select_confirm_dialog(_msg)
                    if res == False:
                        return
                    else:
                        cur_top_node = list(all_targets.values()).pop()
                else:
        ...
        #       - set .fur path / image search path
        #       - assign material
        for y_shape_name, yeti_node_in_json in list(grm_cache_info.items()):
            for ns_info, root_trf_name in all_targets.items():
                
                cur_ns_name     = ns_info
                only_ns_number  = cur_ns_name.split('_')[-1]
                key_yetishape   = y_shape_name
                
                if key_yetishape not in yeti_json_info:
                    continue


                grm_fullpath        = yeti_node_in_json.get("GRM_DATA")[0]                
                img_search_path     = yeti_node_in_json.get("ISPATH")
                hair_mat            = yeti_node_in_json.get("MAT")
                render_density      = yeti_node_in_json.get("RDENSTIY")
                with_curve          = yeti_node_in_json.get("IS_CURVE_VER")
                ...
                for shape_texRef_info in shape_texRef_pair_list:
                    tar_shape_info  = shape_texRef_info.get("EACH_shape")
                    tex_ref         = shape_texRef_info.get("EACH_texRef")
                    if cur_ns_name not in all_targets:
                        ani_nses = "/".join(list(all_targets.keys()))
                        _msg = f"fur 캐쉬와 Ani 캐쉬의 namespace가 다릅니다.\nFur cache : {cur_ns_name}\nAni cache:{ani_nses}\n\nfur 캐쉬파일 기준으로 진행하시겠습니까?"
                        res = select_confirm_dialog(_msg)
                    ...
                    YT.connect_texRef_2_tarShape(tex_ref, tar_basemesh)
                    y_input_shape_list.append(tar_basemesh)
                ...
                trf_yeti            = cmds.listRelatives(created_yeti, p=True)[0]
                
                inherit_trs_attr    = created_yeti     + ".inheritsTransform"
                ...
                # cmds.setAttr(inherit_trs_attr, False)
                ...
                cmds.setAttr(cache_file_attr, grm_fullpath, typ="string")
                cmds.setAttr(file_mode_attr, 1)
                cmds.setAttr(override_attr, True)
