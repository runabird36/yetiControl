...
class MdlExporter(Exporter):
    ...
    def pre_execute(self, targets, options):
        ...
        if neon.does_MASH_exists() == False:
            self.usd_pub_path = self.get_pub_paths('usd')
            if pub_lastver != "None":
                self.usd_pub_path = re.sub(r"/v\d+/", "/"+pub_lastver+"/", self.usd_pub_path)
                self.usd_pub_path = re.sub(r"\_v\d+\.usd$", "_"+pub_lastver+".usd", self.usd_pub_path)
            
            all_path.append(self.usd_pub_path)
    ...
    def execute(self):
        ...
        if neon.does_MASH_exists() == False:
            # usd file publish
            ass_pub_dir_path = os.path.dirname(self.usd_pub_path)
            if not os.path.exists(ass_pub_dir_path):
                os.makedirs(ass_pub_dir_path)

            if self.PUB_WITH_MAT == False:
                ass_opt_list    = ['shape']
            elif self.PUB_WITH_MAT == True:
                ass_opt_list    = ['option', 'shape', 'shader', 'override', 'driver', 'filter', 'colorManager', 'operators']
            else:
                ass_opt_list    = ['shape']
            neon.export_usd(self.usd_pub_path, self.target, ass_opt_list)

            pub_targets.append(self.usd_pub_path)
