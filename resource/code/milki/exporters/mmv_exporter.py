...
def get_nuke_scene_path(tar_vernum :int) -> str:
    
    def find_right_file(file_list :list, tar_num :int) -> str:
        str_num = str(tar_num).zfill(3)
        search_tar = f"_v{str_num}.nk"
        for _file in file_list:
            if search_tar in _file:
                return _file
        return ""
        
    scene_dir = LUCY.get_scenes_dir()
    nuke_dev_dir = f"{scene_dir}/nuke"
    if os.path.exists(nuke_dev_dir) == False:
        return ""
    scene_list = os.listdir(nuke_dev_dir)
    
    tar_file = find_right_file(scene_list, tar_vernum)
    
    return f"{nuke_dev_dir}/{tar_file}"
...
class MMVExporter(Exporter):
    def __init__(self, m_sg_tk, p_dialog):
        ...
        self.options = None
        self.targets = None
        self.export_order_dict = {'playblast' : PlayblastItem(), 'alembic cache': AbcItem(self), 'arnold scene source (ass)':ASSItem(),'maya':MayaItem()}
    ...
    def pre_execute(self, targets, options):
        ...
        nuke_version = self.options["Description 에 입력할 nuke version"][0]
        nuke_version = int(re.search(r"\d+", nuke_version).group())
        
        nuke_scene_path = get_nuke_scene_path(nuke_version)
        
        self.panel.set_desc(nuke_scene_path)
